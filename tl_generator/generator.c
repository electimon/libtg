				...
				strstr(m->args[i].name, "flags"))
		{
			...
			char *p;
			{
						...
						(p = strstr(m->args[i].type, "ector<")))
				{
					...
					fputs(STR(buf, BLEN,
					  //"\t\t*buf = buf_add(buf->data + 4, buf->size - 4); // skip vecrtor definition\n"
					  "\t\tobj->%s_len = *(int *)(buf->data);\n"
					  "\t\t*buf = buf_add(buf->data + 4, buf->size - 4);\n"
					, m->args[i].name)
					, g->deserialize_table_c);
					...
					{
						fputs(STR(buf, BLEN,
							"\t\tobj->%s_ = (int *)MALLOC(obj->%s_len * sizeof(int), return NULL);\n"
							"\t\tint i;\n"
							"\t\tfor(i=0; i<obj->%s_len; ++i){\n"
							"\t\t\tobj->%s_[i] = *(int *)buf->data;\n"
							"\t\t\t*buf = buf_add(buf->data + 4, buf->size - 4);\n"
							"\t\t}\n"
					...
					{
						fputs(STR(buf, BLEN,
							"\t\tobj->%s_ = (long *)MALLOC(obj->%s_len * sizeof(long), return NULL);\n"
							"\t\tint i;\n"
							"\t\tfor(i=0; i<obj->%s_len; ++i){\n"
							"\t\t\tobj->%s_[i] = *(long *)buf->data;\n"
							"\t\t\t*buf = buf_add(buf->data + 8, buf->size - 8);\n"
							"\t\t}\n"
						, m->args[i].name, m->args[i].name, m->args[i].name, m->args[i].name)
						, g->deserialize_table_c);
					...
					{
						fputs(STR(buf, BLEN,
							"\t\tobj->%s_ = (char **)MALLOC(obj->%s_len * sizeof(char *), return NULL);\n"
							"\t\tint i;\n"
							"\t\tfor(i=0; i<obj->%s_len; ++i){\n"
							"\t\t\tbuf_t str = deserialize_string(buf);\n"
							"\t\t\tobj->%s_[i] = strdup((char *)str.data);\n"
							"\t\t}\n"
						, m->args[i].name, m->args[i].name, m->args[i].name, m->args[i].name)
						, g->deserialize_table_c);
					}
